...
do
    ...
    echo -e "${sample}\t${value}" >> other_data/cladocopium_mapped_number.txt
done
...
do
    sample=$(basename "$file" .sam)
    value=$(samtools flagstat "$file" | awk 'NR == 7 {print $1}')
    echo -e "${sample}\t${value}" >> other_data/durusdinium_mapped_number.txt
done
...
do
    sample=$(basename "$file" .sam)
    value=$(samtools flagstat "$file" | awk 'NR == 7 {print $1}')
    echo -e "${sample}\t${value}" >> other_data/symbiodinium_mapped_number.txt
done
...
do
    sample=$(basename "$file" .sam)
    value=$(samtools flagstat "$file" | awk 'NR == 7 {print $1}')
    echo -e "${sample}\t${value}" >> other_data/acropora_mapped_number.txt
done